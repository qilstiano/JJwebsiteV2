    ...
    <main className="w-full pt-8 flex flex-col bg-gradient-to-r from-light-violet to-extra-light-violet min-h-screen">
      <section className="bg-[url('/images/AboutUs/OurVisionMasthead.jpg')] bg-cover bg-center relative">
        ...
        <div className='max-w-6xl mx-auto relative z-30 min-h-[350px] flex flex-col justify-center'>
          <h2 className='font-bold text-white text-[72px]'>Our Mission:</h2>
          <h1 className='font-bold text-white text-[50px]'>To create a safe space for youths to discuss social issues</h1>
        </div>
      </section>
      ...
      <section className='max-w-6xl mx-auto pt-2'>
        <div className='flex gap-1 pb-4'>
          ...
          <span className='font-bold text-lg pt-0.5'>{`>`}</span>
          <a href={`about/${TeamMember.slug}`} className='text-[#306054] text-base text-center py-1.5 px-3 font-bold'>{TeamMember.name}</a>
        </div>
        <div className='flex gap-10 items-start pb-6'>
          <div className='flex flex-col items-center gap-2.5'>
            <Image alt="About Us Image" src={`/images/AboutUs/${TeamMember.imgSrc}`} width={350} height={350} />
            <Image alt="LinkedIn Image" src={`/images/AboutUs/LinkedIn.png`} width={40} height={40} />
          </div>
          <div className='font-normal text-black text-base flex flex-col gap-5 w-[60%] pt-6'>
